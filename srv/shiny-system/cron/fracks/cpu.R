setwd("/srv/shiny-system/cron/fracks")
I <- 0
for (i in 1:60) {
    ...
    if (length(id) > 0) {
        ...
        for (i in 1:length(dat$PID)) {
            ...
            # Liveserver.
            # dat$app[i] <- regmatches(lsof, regexec("usr/local/chroot/csshiny/devopenair.citizensense.net/fracks/(.*)", lsof))[[1]][2]

            # Localhost.
            dat$app[i] <- regmatches(lsof, regexec("srv/shiny-server/fracks/(.*)", lsof))[[1]][2]
